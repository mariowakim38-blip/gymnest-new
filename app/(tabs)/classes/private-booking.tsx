...
export default function AdminPanel() {
  ...
  const [attendanceType, setAttendanceType] = useState<'class' | 'private'>('class');
  const [privateAttendanceSessions, setPrivateAttendanceSessions] = useState<any[]>([]);
  const [privateAttendanceBookings, setPrivateAttendanceBookings] = useState<any[]>([]);
  const [showPrivateMakeupModal, setShowPrivateMakeupModal] = useState<boolean>(false);
  const [privateMakeupSearch, setPrivateMakeupSearch] = useState<string>('');
  const [selectedPrivateMakeupBooking, setSelectedPrivateMakeupBooking] = useState<any>(null);
  const [privateMakeupNote, setPrivateMakeupNote] = useState<string>('');
  ...
  const refreshPrivateAttendanceSessions = async () => {
    ...
    const { data: privateBookingsData, error: privateBookingsError } = await supabase
    ...
    const mappedPrivateBookings = (privateBookingsData ?? []).map((booking: any) => {
      const parent = (profilesData ?? []).find(
        (profile: any) => String(profile.id) === String(booking?.profile_id)
      );
      const child = (childrenData ?? []).find(
        (item: any) => String(item.id) === String(booking?.child_id)
      );
    ...
    const mappedPrivateSessions = (sessionsData ?? []).map((session: any) => {
      const privateBooking = mappedPrivateBookings.find(
        (booking: any) => String(booking.id) === String(session.private_booking_id)
      );
      ...
      return {
        ...
        child: privateBooking?.child,
      };
    });

    setPrivateAttendanceBookings(mappedPrivateBookings);
...
const handleCreateParentAccount = async () => {
  ...
  const handleMarkPrivateAttendance = async (sessionId: string, attended: boolean) => {
    const { error } = await supabase
      .from('private_booking_sessions')
      .update({
        attended,
    ...
    if (error) {
      ...
      const errorMessage = error.message || 'Failed to update private attendance.';
      if (Platform.OS === 'web') alert(errorMessage);
  ...
  const privateMakeupBookingOptions = privateAttendanceBookings
    .filter((booking: any) => booking.status !== 'cancelled')
    .filter((booking: any) => {
      const search = privateMakeupSearch.trim().toLowerCase();
      if (!search) return true;

      return (
        String(booking.child?.name || '').toLowerCase().includes(search) ||
        String(booking.parent?.name || '').toLowerCase().includes(search) ||
        String(booking.parent?.phone_number || '').toLowerCase().includes(search) ||
        String(booking.description || '').toLowerCase().includes(search) ||
        String(booking.title || '').toLowerCase().includes(search)
      );
    });

  const handleOpenPrivateMakeupModal = () => {
    if (!selectedAttendanceDate) {
      const msg = 'Select the private attendance date first.';
      if (Platform.OS === 'web') alert(msg);
      else Alert.alert('Select date', msg);
      return;
    }

    setPrivateMakeupSearch('');
    setSelectedPrivateMakeupBooking(null);
    setPrivateMakeupNote('');
    setShowPrivateMakeupModal(true);
  };

  const handleAddPrivateMakeupSession = async () => {
    if (!selectedAttendanceDate || !selectedPrivateMakeupBooking) {
      const msg = 'Select a private booking before adding a make-up session.';
      if (Platform.OS === 'web') alert(msg);
      else Alert.alert('Missing private booking', msg);
      return;
    }
    ...
    const { error } = await supabase
      .from('private_booking_sessions')
      .insert({
        private_booking_id: selectedPrivateMakeupBooking.id,
        session_date: selectedAttendanceDate,
        attended: true,
        attendance_marked_at: new Date().toISOString(),
        note: privateMakeupNote.trim() || 'Private make-up session',
      });

    if (error) {
      const errorMessage = error.message || 'Failed to add private make-up session.';
      if (Platform.OS === 'web') alert(errorMessage);
      else Alert.alert('Error', errorMessage);
      return;
    }
    ...
    await refreshPrivateAttendanceSessions();
    setShowPrivateMakeupModal(false);
    setSelectedPrivateMakeupBooking(null);
    setPrivateMakeupSearch('');
    setPrivateMakeupNote('');
  };
  ...
  <TouchableOpacity style={styles.addButton} onPress={handleOpenCreateUserModal}>
    <Plus color={Colors.white} size={20} />
          ...
          <View style={styles.section}>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
              <TouchableOpacity
                style={[
                  ...
                  attendanceType !== 'class' && styles.attendanceFilterButtonSecondary,
                ]}
              ...
              <TouchableOpacity
                style={[
                  styles.attendanceFilterButton,
                  attendanceType !== 'private' && styles.attendanceFilterButtonSecondary,
                ]}
            ...
            {attendanceType === 'private' && (
              <View>
                ...
                <TouchableOpacity
                  style={[styles.smallActionButton, { alignSelf: 'flex-start', marginBottom: 12 }]}
                  onPress={handleOpenPrivateMakeupModal}
                  activeOpacity={0.85}
                >
                  <Text style={styles.smallActionButtonText}>+ Private Make-Up Session</Text>
                </TouchableOpacity>
      ...
      {showPrivateMakeupModal && (
        <View style={styles.modal}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Add Private Make-Up Session</Text>
              <TouchableOpacity onPress={() => setShowPrivateMakeupModal(false)}>
                <X color={Colors.darkGray} size={24} />
              </TouchableOpacity>
            </View>
            ...
            <ScrollView style={styles.form}>
              ...
              <Text style={styles.makeupContextText}>
                {selectedAttendanceDate ? formatAttendanceDate(selectedAttendanceDate) : 'No date selected'}
              </Text>

              <Text style={styles.label}>Search Private Booking</Text>
              <TextInput
                style={styles.input}
                value={privateMakeupSearch}
                onChangeText={setPrivateMakeupSearch}
                placeholder="Type child, parent, phone, or description"
              />

              <View style={styles.makeupResultsBox}>
                {privateMakeupBookingOptions.slice(0, 10).map((booking: any) => {
                  const isSelected = selectedPrivateMakeupBooking?.id === booking.id;

                  return (
                    <TouchableOpacity
                      key={booking.id}
                      style={[styles.makeupResultCard, isSelected && styles.makeupResultCardActive]}
                      onPress={() => setSelectedPrivateMakeupBooking(booking)}
                      activeOpacity={0.85}
                    >
                      <Text style={[styles.makeupResultName, isSelected && styles.makeupResultNameActive]}>
                        {booking.child?.name || 'Unknown student'}
                      </Text>
                      <Text style={[styles.makeupResultParent, isSelected && styles.makeupResultParentActive]}>
                        Parent: {booking.parent?.name || 'Unknown parent'}
                      </Text>
                      {!!booking.description && (
                        <Text style={[styles.makeupResultParent, isSelected && styles.makeupResultParentActive]}>
                          {booking.description}
                ...
                {privateMakeupBookingOptions.length === 0 && (
                  <Text style={styles.emptyStateText}>No private bookings found.</Text>
                )}
              </View>
              ...
              <TextInput
                ...
                value={privateMakeupNote}
                onChangeText={setPrivateMakeupNote}
                placeholder="Example: Private make-up for missed session"
                multiline
                numberOfLines={3}
              />

              <View style={styles.modalActions}>
                <TouchableOpacity style={[styles.button, styles.buttonSecondary]} onPress={() => setShowPrivateMakeupModal(false)}>
                  <Text style={styles.buttonSecondaryText}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.button, styles.buttonPrimary, !selectedPrivateMakeupBooking && styles.buttonDisabled]}
                  onPress={handleAddPrivateMakeupSession}
                  disabled={!selectedPrivateMakeupBooking}
                >
